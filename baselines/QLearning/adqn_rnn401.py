# adqn_rnn322.py + Attention mechanism + monotonicity
...
class MonotonicDense(nn.Module):
    """
    Dense layer with strictly non-negative weights to enforce monotonicity w.r.t. inputs.
    Weights are parameterized via softplus to guarantee positivity.
    """
    features: int
    use_bias: bool = True
    ...
    def __call__(self, inputs):
        ...
        # positive_kernel = jax.nn.softplus(kernel) + self.epsilon
        positive_kernel = jnp.abs(kernel) 
        y = jnp.einsum("...i,ij->...j", inputs, positive_kernel)
        if self.use_bias:
...
class MixingNetwork(nn.Module):
    ...
    def __call__(self, hidden, joint_observation, state, joint_action, dones):
        ...
        embedding = MonotonicDense(
            self.mixer_dim * self.num_agents + self.mixer_dim,
            kernel_init=orthogonal(self.init_scale),
            bias_init=constant(0.0),
        )(input)
        embedding = jax.nn.leaky_relu(embedding, negative_slope=1e-2)
        embedding = MonotonicDense(
            self.mixer_dim * self.num_agents + self.mixer_dim,
            kernel_init=orthogonal(self.init_scale),
            bias_init=constant(0.0),
...
def single_run(config):
    ...
    alg_name = config.get("ALG_NAME", "adqn_rnn2_attn_mono")
...
def tune(default_config):
    ...
    alg_name = default_config.get("ALG_NAME", "adqn_rnn2_attn_mono") 
