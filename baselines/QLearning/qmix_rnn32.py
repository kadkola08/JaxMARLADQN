        ...
        def _update_step(runner_state, unused):
            ...
            def _learn_phase(carry, _):
                ...
                def _agent_loss_fn(params):
                    ...
                    loss = jnp.mean(
                        (chosen_action_q_vals - jax.lax.stop_gradient(qmix_target)) ** 2
                    )

                    return loss, chosen_action_q_vals.mean()

                (agent_loss, agent_qvals), agent_grads = jax.value_and_grad(_loss_fn, has_aux=True)(
                    train_state.params
                )
                agent_grads = {
                    'agent': agent_grads['agent'],
