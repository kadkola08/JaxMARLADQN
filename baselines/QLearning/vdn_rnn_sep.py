        ...
        def _update_step(runner_state, unused):
            ...
            def _learn_phase(carry, _):
                ...
                for i, agent in enumerate(env.agents):
                    ...
                    actions = _actions[i]
                    rewards = _rewards[i]
                    avail_actions = _avail_actions[i]
                    ...
                    _, q_next_target_agent = network_class.apply(
                        train_states[agent].target_network_params,
                        init_hs,
                        obs,
                        dones,
                    ) 
                    q_next_target[agent] = q_next_target_agent

                    _, q_vals_agent = network_class.apply(
                        train_states[agent].params,
                        init_hs,
                        obs,
                        dones
                    )
                    q_vals[agent] = q_vals_agent

                q_next_target_ = batchify(q_next_target)
                q_vals_ = batchify(q_vals)
                ...
                valid_q_vals = q_vals_ - (unavailable_actions * 1e10)
                ...
                for i, agent in enumerate(env.agents):
                    ...
                    def _loss_fn(params):
                        ...
                        chosen_action_q_vals_agent = jnp.take_along_axis(
                            q_vals_agent,
                            actions[..., np.newaxis],
                            axis=-1,
                        ).squeeze(-1)
                        chosen_action_q_vals_agent = chosen_action_q_vals_agent[:-1]

                        loss = jnp.mean(
                            (chosen_action_q_vals_agent - jax.lax.stop_gradient(vdn_target)) ** 2
                        )

                        return loss, chosen_action_q_vals_agent.mean()

                    (agent_loss, agent_qvals), grads = jax.value_and_grad(_loss_fn, has_aux=True)(
                        train_states[agent].params
                    )

                    new_train_state = train_states[agent].apply_gradients(grads=grads)
                    new_train_state = new_train_state.replace(
                        grad_steps=new_train_state.grad_steps + 1,
                    )

                    new_train_states[agent] = new_train_state
                    agent_losses[agent] = agent_loss
                    agent_q_vals[agent] = agent_qvals

                avg_loss = jnp.mean(jnp.array([agent_losses[a] for a in env.agents]))
                avg_qvals = jnp.mean(jnp.array([agent_q_vals[a] for a in env.agents]))
...
def single_run(config):
    ...
    alg_name = config.get("ALG_NAME", "vqn_rnn_sep")
...
def tune(default_config):
    ...
    alg_name = default_config.get("ALG_NAME", "vdn_rnn_sep") 
