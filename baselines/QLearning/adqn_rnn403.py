...
class MixingNetwork(nn.Module):
    """
    Attention-based mixing network that is permutation-equivariant.
    Processes agent observations through self-attention before mixing.
    """
    embedding_dim: int
    mixer_dim: int = 256
    init_scale: float = 1.0
    ...
    def __call__(self, hidden, joint_observation, state, joint_action, dones):
        ...
        # Process each agent's observations independently first
        ...
        )(joint_observation)  # (n_agents, batch, time, embedding_dim)
        agent_embeddings = nn.relu(agent_embeddings)
        
        # Reshape for attention: (batch*time, n_agents, embedding_dim)
        agent_embeddings = agent_embeddings.transpose(1, 2, 0, 3)  # (batch, time, n_agents, embedding_dim)
        agent_embeddings = agent_embeddings.reshape(-1, n_agents, self.embedding_dim)
        
        # Multi-head self-attention among agents
        ...
        queries = nn.Dense(
            ...
            kernel_init=orthogonal(self.init_scale),
            bias_init=constant(0.0)
        )(agent_embeddings)  # (batch*time, n_agents, embedding_dim)
        ...
        queries = queries.reshape(-1, n_agents, self.n_heads, head_dim)
        keys = keys.reshape(-1, n_agents, self.n_heads, head_dim)
        values = values.reshape(-1, n_agents, self.n_heads, head_dim)
        
        # Compute attention scores for each head
        scores = jnp.einsum('bnhd,bmhd->bhnm', queries, keys) / jnp.sqrt(head_dim)
        attention_weights = nn.softmax(scores, axis=-1)
        
        # Apply attention
        attended = jnp.einsum('bhnm,bmhd->bnhd', attention_weights, values)
        
        # Concatenate heads and project
        attended = attended.reshape(-1, n_agents, self.embedding_dim)
        attended = nn.Dense(
            self.embedding_dim,
            kernel_init=orthogonal(self.init_scale),
            bias_init=constant(0.0)
        )(attended)
        attended = nn.relu(attended)
        
        # Permutation-invariant pooling across agents
        # Use multiple aggregations for richer representation
        pooled_mean = jnp.mean(attended, axis=1)  # (batch*time, embedding_dim)
        pooled_max = jnp.max(attended, axis=1)
        pooled_std = jnp.std(attended, axis=1)
        
        # Combine pooled features
        pooled = jnp.concatenate([pooled_mean, pooled_max, pooled_std], axis=-1)
        pooled = pooled.reshape(batch_size, time_steps, -1)
        ...
        state_embed = nn.relu(state_embed)
        state_embed = nn.Dense(
            256,
            kernel_init=orthogonal(self.init_scale),
            bias_init=constant(0.0)
        ...
        joint_action_embed = nn.Dense(
            256,
            kernel_init=orthogonal(self.init_scale),
            bias_init=constant(0.0)
        )(joint_action)
        joint_action_embed = nn.relu(joint_action_embed)
        
        # Combine everything for RNN input
        combined = jnp.concatenate([pooled, state_embed, joint_action_embed], axis=-1)
        
        # Process through RNN for temporal dependencies
        rnn_in = (combined, dones)
...
def make_train(config, env):
    ...
    def train(rng):
        ...
        attention_output_dim = 3 * config["HIDDEN_SIZE"] + 256 + 256
...
def single_run(config):
    ...
    alg_name = config.get("ALG_NAME", "adqn_rnn2")
