...
def make_train(config, env):
    ...
    def train(rng):
        ...
        wrapped_env = CTRolloutManager(
                env, batch_size=config["NUM_ENVS"], preprocess_obs=False# check if this is set to true only for partially observable envs
        )
        test_env = CTRolloutManager(
            env, batch_size=config["TEST_NUM_ENVS"], preprocess_obs=False # check if this is set to true only for partially observable envs
        ...
        def create_agent(rng):
            ...
            init_x = (
                ...
                ),  # (time_step, batch_size, env_num?, obs_size)
