"""
VDN (Value Decomposition Networks) with CNN+RNN architecture implementation
...
class CNN(nn.Module):
    """CNN module for processing spatial observations."""
    activation: str = "relu"
    hidden_size: int = 64
    ...
    def __call__(self, x):
        ...
        # Reshape preserving original batch dimensions
        new_shape = original_batch_dims + (-1,)
...
class CNNRNNVDNNetwork(nn.Module):
    """CNN+RNN network for Value Decomposition Networks."""
    action_dim: int
    hidden_dim: int
    ...
    def __call__(self, hidden, obs, dones):
        # Process observation through CNN to get spatial features
        cnn_embedding = CNN(hidden_size=self.hidden_dim)(obs)

        # Pass CNN embedding through RNN for temporal processing
        rnn_in = (cnn_embedding, dones)
        hidden, embedding = ScannedRNN()(hidden, rnn_in)

        # Output Q-values from the final embedding
        q_vals = nn.Dense(
            self.action_dim,
...
def make_train(config, env):
    ...
    def train(rng):
        ...
        def create_agent(rng):
            ...
            network_params = network.init(rng, init_hs, init_x, init_dones)
        ...
        buffer = fbx.make_trajectory_buffer(
            ...
            min_length_time_axis=int(config["BUFFER_BATCH_SIZE"]),
        ...
        def _update_step(runner_state, unused):
            ...
            def _learn_phase(carry, _):
                ...
                # Get Q-values from target network for next observations
                ...
                # VDN logic: Sum Q-values across agents for joint action-value function
                def _loss_fn(params):
                    # Get Q-values from policy network for current observations
                    ...
                    chosen_action_q_vals = jnp.take_along_axis(
                        ...
                        _actions[..., np.newaxis],
                        axis=-1,
                    ).squeeze(-1)  # (n_agents, batch_size, time)

                    # Handle invalid actions in next state
                    unavailable_actions = 1 - _avail_actions
                    valid_q_vals = q_next_target - (unavailable_actions * 1e10)
                    max_q_next = jnp.max(valid_q_vals, axis=-1)  # (n_agents, batch_size, time)

                    # VDN: Sum the max Q-values across agents
                    vdn_max_q_next = jnp.sum(max_q_next, axis=0)  # (batch_size, time)

                    # Calculate target Q-values (reward + discount * max_q_next)
                    # Using __all__ for team reward
                    target = (
                        minibatch.rewards["__all__"][:-1]
                        + (1 - minibatch.dones["__all__"][:-1]) * config["GAMMA"] * vdn_max_q_next[1:]
                    )

                    # VDN: Sum the chosen action Q-values across agents
                    vdn_chosen_q = jnp.sum(chosen_action_q_vals[:, :-1], axis=0)  # (batch_size, time-1)

                    # Calculate MSE loss
                    loss = jnp.mean(
                        (vdn_chosen_q - jax.lax.stop_gradient(target)) ** 2
                    )

                    return loss, vdn_chosen_q.mean()
