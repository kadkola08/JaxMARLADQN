...
class MixingNetwork(nn.Module):
    ...
    def __call__(self, hidden, state, joint_action, dones):
        # joint_observation shape (26, 32, 63)
        # state shape (26, 32, 54)
        # joint_action shape (26, 32, 15)

        # x = state
        # x = nn.Dense(32, kernel_init=orthogonal(self.init_scale))(x)
        # x = nn.relu(x)
        # encoded_state = nn.Dense(16, kernel_init=orthogonal(self.init_scale))(x)
        # encoded_state = nn.relu(encoded_state)

        # input = jnp.concatenate([joint_observation, encoded_state, joint_action], axis=-1)  # (time_steps, batch_size, 63+54+15)
        input = jnp.concatenate([state, joint_action], axis=-1)  # (time_steps, batch_size, 63+54+15)
...
class MixingNetwork(nn.Module):
    ...
    def __call__(self, joint_observation, state, joint_action):
        ...
        # time_steps, batch_size, _ = joint_observation.shape
        input = jnp.concatenate([joint_observation, state, joint_action], axis=-1)  # (time_steps, batch_size, 63+54+15)
        ...
        # mix
        x = nn.Dense(self.embedding_dim)(input)
        x = nn.relu(x)
        q_tot = nn.Dense(1)(x)
...
def make_train(config, env):
    ...
    def train(rng):
        ...
        def create_agent(rng):
            ...
            init_dones = jnp.zeros((1, len(env.agents), 1)) # (time_step, batch_size, len(env.agents))
            # mixer_params = mixer.init(_rng, init_mixer_hs, init_jt_obs, init_state, init_jt_act, init_mixer_x[1])
            mixer_params = mixer.init(_rng, init_mixer_hs, init_state, init_jt_act, init_dones)
            ...
            #     init_value=0.001,
            #     end_value=0.00003,
            #     transition_steps=int((config["NUM_EPOCHS"]) * config["NUM_UPDATES"] / 3),
            # )
        ...
        def _update_step(runner_state, unused):
            ...
            def _learn_phase(carry, _):
                ...
                # def _loss_fn(agent_not_mixer, params):
                ...
                def _loss_fn(params):
                    ...
                    # print(type(minibatch.actions))
                    # print(jnp.shape(minibatch.actions))
                    # print("Minibatch actions: ", minibatch.actions)     # Shape: (26, 32)
                    # print("====================================")


                    # CALCULATING Q_TOT USING JOINT_OBS, STATE AND JOINT_ACTION
                    one_hot_actions = {}
                    for agent, action_values in minibatch.actions.items():
                    ...
                    # print("One hot actions: ", one_hot_actions)     # Shape: (26, 32, 5)
                    # print("====================================")
                    ...
                    # print("====================================")
                    # joint_obs = jnp.concatenate

                    # print("Observations: ", minibatch.obs)      
                    # obs['__all__']     --->   Shape: (26,32,54)
                    # obs['agent_0']     --->   Shape: (26,32,21)
                    # obs['agent_1']     --->   Shape: (26,32,21)
                    # obs['agent_2']     --->   Shape: (26,32,21)
                    ...
                    # print("Joint observation: ", joint_observation)


                    # hidden, joint_observation, state, joint_action, dones
                    # breakpoint()
                    ...
                    mixer_loss = jnp.mean((q_tot_vals[:-1] - jax.lax.stop_gradient(q_tot_target)) ** 2)


                    # print("Shape of target:", jnp.shape(target))    # shape: (3, 25, 32)
                    # print("Shape of q_tot_target:", jnp.shape(q_tot_target))    # shape: (25, 32)

                    # target_for_all_agents = jnp.repeat(q_tot_target, len(env.agents), axis=0)
                    target_for_all_agents = jnp.tile(q_tot_target, (len(env.agents), 1, 1))
                    # print("Shape of target_for_all_agents:", jnp.shape(target_for_all_agents))    # shape: (3, 25, 32)
                    ...
                    # )
                    loss = jnp.mean(
                        # (chosen_action_q_vals - jax.lax.stop_gradient(target_for_all_agents)) ** 2
                    ...
                    return mixer_loss, loss, chosen_action_q_vals.mean(), q_tot_vals.mean()
                
                def loss_fn_wrapper(params):
                    mixer_loss, loss, qvals, q_tot_vals = _loss_fn(params)
                    return loss, (mixer_loss, qvals, q_tot_vals)  # Keep mixer_loss as auxiliary output

                def mixer_loss_fn_wrapper(params):
                    mixer_loss, loss, qvals, q_tot_vals = _loss_fn(params)
                    return mixer_loss, (loss, qvals, q_tot_vals)  # Keep loss as auxiliary output

                (loss, (mixer_loss, qvals, q_tot_vals)), agent_grads = jax.value_and_grad(loss_fn_wrapper, has_aux=True)(
                    train_state.params
                    # train_state.params['agent']
                )
                (mixer_loss, (loss, qvals, q_tot_vals)), mixer_grads = jax.value_and_grad(mixer_loss_fn_wrapper, has_aux=True)(
                    train_state.params
                    # train_state.params['mixer']
                )

                # print("agent_grads: ", agent_grads)
                # print("====================================")
                
                # print("mixer_grads: ", mixer_grads)

                all_grads = {'agent': agent_grads['agent'], 'mixer': mixer_grads['mixer']}
                # all_grads = {'agent': agent_grads, 'mixer': mixer_grads}
                
                # print("all_grads: ", all_grads)
                # print("====================================")
                # Extract relevant gradients
                # agent_grads = {'agent': agent_grads['agent']['params']}  # Keep only "agent" gradients
                # mixer_grads = {'mixer': mixer_grads['mixer']['params']}  # Keep only "mixer" gradients


                # Update train_state separately
                # train_state = train_state.apply_gradients(grads=agent_grads)
                # train_state = train_state.apply_gradients(grads=mixer_grads)
                train_state = train_state.apply_gradients(grads=all_grads)

                # train_state = train_state.apply_gradients(grads=grads)
                train_state = train_state.replace(
                    grad_steps=train_state.grad_steps + 1,
                )
                return (train_state, rng), (mixer_loss, loss, qvals, q_tot_vals)
            ...
            (train_state, rng), (mixer_loss, loss, qvals, q_tot_vals) = jax.lax.cond(
                is_learn_time,
            ...
            metrics = {
                ...
                # Rupali: You can track mixer_loss.mean() here
                # Rupali: You can track q_tot_vals.mean() here
            }
...
def single_run(config):
    ...
    alg_name = config.get("ALG_NAME", "adqn_rnn")
...
def tune(default_config):
    ...
    alg_name = default_config.get("ALG_NAME", "adqn_rnn") 
