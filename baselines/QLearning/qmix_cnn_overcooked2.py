...
class MixingNetwork(nn.Module):
    ...
    def __call__(self, q_vals, states):
        n_agents, batch_size = q_vals.shape
        ...
        w_1 = jnp.abs(w_1.reshape(batch_size, n_agents, self.embedding_dim)) # + 1e-3
        b_1 = b_1.reshape(batch_size, 1, self.embedding_dim)
        w_2 = jnp.abs(w_2.reshape(batch_size, self.embedding_dim, 1)) # + 1e-3
...
def make_train(config, env):
    ...
    def train(rng):
        ...
        def create_agent(rng):
            ...
            init_x = jnp.zeros((len(env.agents), 1))
            _obs, _env_state = wrapped_env.batch_reset(_rng)
            state_size = _obs["__all__"].shape[-1]
            # state_size = env.observation_space("__all__").shape[-1]
        ...
        def _update_step(runner_state, unused):
            ...
            def _learn_phase(carry, _):
                ...
                def _loss_fn(params):
                    ...
                    qmix_next = mixer.apply(train_state.target_network_params['mixer'], q_next, minibatch.second.obs["__all__"])
                    qmix_target = (
                    ...
                    qmix = mixer.apply(params['mixer'], chosen_action_q_vals, minibatch.first.obs["__all__"])
                    loss = jnp.mean(
