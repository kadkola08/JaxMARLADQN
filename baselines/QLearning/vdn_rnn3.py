    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            def _step_env2(carry, _):
                ...
                for i, agent in enumerate(env.agents):
                    obs = last_obs[agent][np.newaxis, :, :] 
                    dones = last_dones[agent][np.newaxis, :]

                    agent_hs = hs_dict[agent]
                    new_hs, q_vals = networks[agent].apply(
                    ...
                    # Temp Fix 
                    actions[agent] = agent_actions.squeeze(0)
                print(actions)
                ...
                return (new_hs_dict, new_obs, dones, new_env_state, rng), (timestep, infos)
            ...
            init_hs = {
                agent: ScannedRNN.initialize_carry(
                    config["HIDDEN_SIZE"], 1, config["NUM_ENVS"]
                ) for agent in env.agents
            }
            ...
            def _learn_phase2(carry, _):
                ...
                _obs = {agent: minibatch.obs[agent] for agent in env.agents}
                _dones = {agent: minibatch.dones[agent] for agent in env.agents}
                _actions = {agent: minibatch.actions[agent] for agent in env.agents}
                _avail_actions = {agent: minibatch.avail_actions[agent] for agent in env.agents}
                ...
                for agent in env.agents:
                    _, q_next = networks[agent].apply(
                        ...
                        init_hs,
                        _obs[agent],# [:, np.newaxis],
                        _dones[agent], # [:, np.newaxis],
                    )
                    agent_qvals_next[agent] = q_next
                ...
                for agent in env.agents:
                    def _loss_fn(params):
                        ...
                        chosen_action_q_vals = jnp.take_along_axis(
                            q_vals, # .squeeze(1), 
                            _actions[agent][..., np.newaxis],
                            axis=-1,
                        ).squeeze(-1)

                        unavailable_actions = 1 - _avail_actions[agent]
                        # valid_q_vals = q_vals.squeeze(1) - (unavailable_actions * 1e10)
                        valid_q_vals = q_vals - (unavailable_actions * 1e10)

                        q_next_per_agent = {}
                        for a in env.agents:
                            q_next_a = jnp.take_along_axis(
                                agent_qvals_next[a], #.squeeze(1),
                                jnp.argmax(valid_q_vals if a == agent else
                                          # agent_qvals_next[a].squeeze(1) - (1 - _avail_actions[a]) * 1e10,
                                          agent_qvals_next[a] - (1 - _avail_actions[a]) * 1e10,
                                          axis=-1)[..., np.newaxis],
                                axis=-1,
                            ).squeeze(-1)
                            q_next_per_agent[a] = q_next_a
                        
                        sum_q_next = jnp.zeros_like(list(q_next_per_agent.values())[0])
                        for a in env.agents:
                            sum_q_next = sum_q_next + q_next_per_agent[a]

                        vdn_target = (
                            minibatch.rewards["__all__"][:-1]
                            + (1 - minibatch.dones["__all__"][:-1])
                            * config["GAMMA"]
                            * sum_q_next[1:]  # Next timestep
                        )

                        loss = jnp.mean(
                            (chosen_action_q_vals[:-1] - jax.lax.stop_gradient(vdn_target)) ** 2
                        )
                    ...
                    q_vals, vdn_target = aux
                    
                    updated_train_state = agent_train_states[agent].apply_gradients(grads=grads)
                    updated_train_state = updated_train_state.replace(
                        grad_steps=updated_train_state.grad_steps + 1,
                    )

                    updated_train_states[agent] = updated_train_state
                    loss_values[agent] = loss
                    qval_values[agent] = q_vals
                    vdn_target_values[agent] = vdn_target

                mean_loss = jnp.mean(jnp.array(list(loss_values.values())))
                mean_qvals = jnp.mean(jnp.array(list(qval_values.values())))
                mean_vdn_target = jnp.mean(jnp.array(list(vdn_target_values.values())))

                return (updated_train_states, rng), (mean_loss, mean_qvals, mean_vdn_target)
            ...
            (agent_train_states, rng), (loss, qvals, vdn_target) = jax.lax.cond(
                ...
                lambda agent_states, rng: jax.lax.scan(
                    _learn_phase2, (agent_states, rng), None, config["NUM_EPOCHS"]
                ),
        ...
        def get_greedy_metrics2(rng, agent_train_states):
            if not config.get("TEST_DURING_TRAINING", True):
                return None     

            def _greedy_env_step2(step_state, unused):
                agent_train_states, env_state, last_obs, last_dones, hstate_dict, rng = step_state
                ...
                for agent in env.agents:
                    ...
                    new_hstate_dict[agent] = new_hs

                    q_vals = q_vals.squeeze(axis=0)
                    valid_actions = test_env.get_valid_actions(env_state.env_state)[agent]
                    agent_actions = get_greedy_actions(q_vals, valid_actions)
                ...
                step_state = (agent_train_states, env_state, obs, dones, new_hstate_dict, rng)
            ...
            init_hstate= {
                agent: ScannedRNN.initialize_carry(
                    config["HIDDEN_SIZE"], 1, config["TEST_NUM_ENVS"]
                ) for agent in env.agents
            }

            step_state, (rewards, dones, infos) = jax.lax.scan(
                _greedy_env_step2, step_state, None, config["TEST_NUM_STEPS"]
            )
