...
def cem_optimization(rng, q_network_apply, params, hidden_states, obs, dones, 
    ...
    """
    n_iterations = cem_config["N_ITERATIONS"]
    n_samples = cem_config["N_SAMPLES"]
    n_top = cem_config["N_TOP"]
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            def _step_env(carry, _):
                ...
                # Compute init_mean based on whether we have previous actions
                init_mean = jnp.where(
                    reset_mask,
                ...
                # Keep actions as array for prev_actions, not the dict
                return (new_hs, new_obs, dones, new_env_state, actions, rng), (timestep, infos)

            # Step the environment using persistent expl_state
            rng, _rng = jax.random.split(rng)
            ...
            # Update expl_state with the final carry (excluding rng)
            expl_state = carry[:5]  # (hs, obs, dones, env_state, prev_actions)
        ...
        init_actions = jnp.zeros((env.num_agents, config["NUM_ENVS"], action_dim))
        expl_state = (init_hs, init_obs, init_dones, env_state, init_actions)
