...
class DuelingCNNRNNQNetwork(nn.Module):
    """
    Dueling RNN Q-Network for QPLEX.
    Outputs both value (V) and advantage (A) streams, then combines them as Q = V + A.
    """
    action_dim: int
    hidden_dim: int
    ...
    def __call__(self, hidden, obs, dones):
        ...
        q_vals = value[..., None] + (advantage - jnp.mean(advantage, axis=-1, keepdims=True))

        return hidden, q_vals


class HyperNetwork(nn.Module):
    """HyperNetwork for generating weights of QPLEX' mixing network."""
...
class QPLEXMixingNetwork(nn.Module):
    ...
    Combines individual agent Q-values using dueling architecture:
    Q_tot = V_tot + A_tot, where V_tot and A_tot are computed separately.
    """
    ...
    def __call__(self, q_vals, states):
        ...
        # Compute V_tot: sum of individual values (or learned combination)
        # For simplicity, we use a learned transformation of the state
        v_tot = nn.Dense(
            1,
            kernel_init=orthogonal(self.init_scale),
            bias_init=constant(0.0),
        )(states)
        v_tot = v_tot.squeeze(-1)  # (time_steps, batch_size)

        # Compute A_tot: mixing network over advantages
        # Hypernetwork for advantage mixing weights
        w_1 = HyperNetwork(
        ...
        # Monotonicity and reshaping for advantage mixing
        ...
        a_tot = jnp.matmul(hidden, w_2) + b_2
        a_tot = a_tot.squeeze()  # (time_steps, batch_size)

        # Combine: Q_tot = V_tot + A_tot
        q_tot = v_tot + a_tot

        return q_tot  # (time_steps, batch_size)
        ...
        def _update_step(runner_state, unused):
            ...
            def _learn_phase(carry, _):
                ...
                def _loss_fn(params):
                    ...
                    qplex_next = mixer.apply(train_state.target_network_params['mixer'], q_next, state)
                    qplex_target = (
                        ...
                        * config["GAMMA"]
                        * qplex_next[1:]  # sum over agents
                    )

                    qplex = mixer.apply(params['mixer'], chosen_action_q_vals, state)[:-1]
                    loss = jnp.mean(
                        (qplex - jax.lax.stop_gradient(qplex_target)) ** 2
                    )

                    return loss, (chosen_action_q_vals.mean(), qplex.mean())
                ...
                return (train_state, rng), (loss, qvals, qplex)
            ...
            (train_state, rng), (loss, qvals, qplex) = jax.lax.cond(
                is_learn_time,
...
def single_run(config):
    ...
    alg_name = config.get("ALG_NAME", "qplex_cnn_rnn")
