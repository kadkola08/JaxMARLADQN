    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            def _learn_phase(carry, _):
                ...
                # Get next state Q-values using target network for all agents
                ...
                # Calculate VDN target (sum of individual Q-values)
                vdn_target = (
                    minibatch.first.rewards["__all__"]
                    + (1 - minibatch.first.dones["__all__"])
                    * config["GAMMA"]
                    * jnp.sum(q_next_chosen, axis=0)  # sum over agents
                )

                # Also calculate QMIX target for comparison or optional use
                q_tot_next = mixer.apply(
                    train_state.target_network_params['mixer'],
                    jnp.transpose(q_next_chosen),  # (batch_size, num_agents)
                    minibatch.second.obs["__all__"]
                ...
                def _loss_fn(params):
                    ...
                    vdn_tot = jnp.sum(chosen_action_q_vals, axis=0)

                    # Also get QMIX value for chosen actions (for comparison)
                    qmix_tot = mixer.apply(
                        params['mixer'],
                        jnp.transpose(chosen_action_q_vals),  # (batch_size, num_agents)
                        minibatch.first.obs["__all__"]
                    )

                    loss = jnp.mean((vdn_tot - jax.lax.stop_gradient(vdn_target)) ** 2)
                ...
                for agent in env.agents:
                    def _agent_loss_fn(params):
                        ...
                        agent_q = chosen_action_q_vals[agent_idx]

                        loss = jnp.mean((agent_q - jax.lax.stop_gradient(vdn_target)) ** 2)

                        return loss, agent_q.mean()
                ...
                # Calculate the global loss for metrics reporting
                (loss, qvals), _ = jax.value_and_grad(_loss_fn, has_aux=True)(
                    train_state.params
                )
                ...
                combined_grads = {
                    ...
                    'mixer': jax.tree.map(lambda x: jnp.zeros_like(x), train_state.params['mixer'])  # Zero gradients for mixer
                }
